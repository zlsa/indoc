...
exports.project = require('./class').class.extend({
  ...
  remove_duplicate_files: function() {
    var filenames = [];
  ...
  get_link_filenames: function(ignore) {
    ...
    for(i=0; i<this.files.length; i++) {
      ...
      // The README already appears on the home page, so it shouldn't
      // appear in the list, ever.
      ...
      filenames.push({
        ...
        separator: ((path.dirname(this.files[i].filename) != last_path_dir) ? true : false),
  ...
  generate_overview: function(callback) {
    ...
    for(i=0; i<this.files.length; i++) {
      ...
      if(this.files[i].filename == this.options.readme) {
        
        (function(f) {
          var filepath = path.join(scope.options.output, 'index.html');
          
          f.generate_output(null, null, true, function(err, data) {
